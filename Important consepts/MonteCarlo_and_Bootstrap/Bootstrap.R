...
ExFunc2 <- function(X ,B ,n){
  med_vec <- c()
  for(i in 1:B){
    Xstar <- sample(X ,n ,replace=TRUE)
    med_vec <- c(med_vec, median(Xstar))
  }
  SE <- sd(med_vec)
  
  hist(med_vec,
       freq = FALSE,
       main = "Bootstrap Median",
       xlab = "Median values",
       ylab = "Density",
       col = "orange",
       border = "black")
  lines(density(med_vec), col='blue', lwd=1.2)
...
ExFunc3 <- function(X,B,alpha,n){
  mean_vec <- c()
  for(i in 1:B){
    Xstar <- sample(X,n,replace = TRUE)
    mean_vec <- c(mean_vec, mean(Xstar, trim = alpha))
  }
  mean_ans = mean(mean_vec)
  var_ans = var(mean_vec)
  
  # Check if mean_vec is valid before plotting
  
  hist(mean_vec,
       breaks = "FD",  # Freedman–Diaconis rule handles skewed data well
       freq = FALSE,
       main = "Bootstrap trimmed mean",
       xlab = "Trimmed mean values",
       ylab = "Density",
       col = "red",
       border = "black")
  lines(density(mean_vec), col = 'blue', lwd = 1.2)

  return(c(mean_ans,var_ans))
}


ExFunc3(x ,1000 ,0.1, 10)

################################################################################
#EXAMPLE 6 p25:
#Data
X<-c(9.15,9.63,10.81,10.61,10.00,9.29,
     10.33,9.40,10.43,10.69,10.77,10.31,
     9.10,10.82,10.34)

ExFunc4 <- function(X, B, n, name){
  vec = c()
  
  if (name == 'mean')
  {
    ExFunc3(X, B, 0, n)[1] -> ans
    return(ans)
  }
  else if (name == 'median')
  {
  ...
  else{ return('Not a valid statistic name!!')}
 
  hist(vec,
       breaks = "FD",  # Freedman–Diaconis rule handles skewed data well
       freq = FALSE,
       main = paste("Bootstrap",name),
       xlab = "Trimmed mean values",
       ylab = "Density",
       col = "red",
       border = "black")
  lines(density(vec), col = 'blue', lwd = 1.2) 
  
  return(SE)
}


ExFunc4(X, 100, 10, "mean")
ExFunc4(X, 100, 10, "median")
ExFunc4(X, 100, 10, "sd")
ExFunc4(X, 100, 10, "var")
